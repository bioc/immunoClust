...
/* trail: labelled fixed zero cluster
...
vs_htrans::t_init(double certainty)
{
	...
	if( !L ) {
		...
		dbg::printf("INIT trail:  %.0lf (%d)", sumN, N );
		...
		if( K > 1 ) {
			...
			for( k=0; k<K; ++k ) {
				if( m[k] > 0.0 && m[k] < zeroY ) {
					zeroY = m[k];
				}
			}
		}
		
		zeroB = -zeroY*certainty;
		dbg::printf("INIT zero: %.1lf / %.0lf (%d)", zeroY, sumN, N );
	}
}
...
vs_htrans::t_grad(double a, double b, double& dfda, double& dfdb)
{
	...
	for(i=0; i<N; ++i) {
		if( (k=*l++) >= 0 ) {
			...
			// t = dlog(dh) = h''(y|a,b)/h'(y|a,b) = -x/(x^2+1), x=a*y+b
			double t = htrans::d_logd(*y-zeroY, a, b);
